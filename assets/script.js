jQuery(document).ready(function ($) {
    ...
    async function processImagesInBackground(topic) {
        ...
        try {
            ...
            // Randomize target count (2 to Max)
            let targetCount = (maxImages > 1) ? Math.floor(Math.random() * (maxImages - 1)) + 2 : 1;
            if (targetCount > maxImages) targetCount = maxImages;
            ...
            // Shuffle headings to pick random insertion points
            ...
            let imagesToProcess = allHeadings.slice(0, targetCount);

            // Sort back by index to process in order (makes progress bar look logical)
            imagesToProcess.sort((a, b) => a.index - b.index);

            console.log(`Found ${headings.length} headings. Targeted ${targetCount} images. Processing:`, imagesToProcess);
            ...
            try {
                ...
                const videoResponse = await $.post(aimatic_writer_vars.ajax_url, {
                    ...
                    nonce: aimatic_writer_vars.nonce,
                    query: searchQuery // Use simplified topic
                });

...
class AIMatic_Campaigns {
    ...
    public function process_campaigns($force = false) {
        ...
        foreach ($campaigns as $id => $campaign) {
            if ($campaign['status'] !== 'active' && !$force) {
                // AIMatic_Logger::log("DEBUG: Skipping Campaign '{$campaign['name']}' (Status: {$campaign['status']})", 'DEBUG');
                continue;
            }
            ...
            $is_due = ($time_since >= $interval);
            
            // AUTO-HEAL: If last_run is 0 (broken) or > now (future), reset it to NOW and skip this run to stop flood.
            if ($last_run <= 0 || $last_run > ($now + 300)) {
                 AIMatic_Logger::log("AUTO-HEAL: Campaign '{$campaign['name']}' had invalid last_run ($last_run). Resetting to NOW and skipping.", 'WARNING');
                 $campaign['last_run'] = $now;
                 // Use direct update to avoid triggering full save hooks if sensitive
                 $all_campaigns = $this->get_campaigns();
                 if (isset($all_campaigns[$id])) {
                     $all_campaigns[$id]['last_run'] = $now;
                     update_option($this->option_name, $all_campaigns);
                 }
                 continue;
            }

            AIMatic_Logger::log("DEBUG: Checking Campaign '{$campaign['name']}': Last Run: " . date('H:i:s', $last_run) . ", Interval: {$interval}s, Time Since: {$time_since}s. Due? " . ($is_due ? "YES" : "NO"), 'DEBUG');

            if (($now - $last_run) >= $interval || $force) {
                // LOCK CHECK: Prevent Race Condition
                $lock_key = 'aimatic_campaign_lock_' . $id;
                if (get_transient($lock_key)) {
                    AIMatic_Logger::log("CRON SKIP: Campaign '{$campaign['name']}' is currently running (Locked).", 'INFO');
                    continue;
                }
                
                // Set Lock (10 minutes max safety expiry)
                set_transient($lock_key, time(), 600);

                $count = isset($campaign['posts_per_run']) ? intval($campaign['posts_per_run']) : 1;
                $count = max(1, $count); // Ensure at least 1
                
                AIMatic_Logger::log("CRON START: Running Campaign '{$campaign['name']}' (Schedule: $schedule). Generating $count articles.", 'INFO');
                
                for ($i = 0; $i < $count; $i++) {
                    // FIX: Reload campaign data to ensure fresh keyword/state for this iteration
                    // This prevents re-using the same keyword if multiple posts are set per run.
                    $fresh_campaigns = $this->get_campaigns();
                    if (isset($fresh_campaigns[$id])) {
                        $this->run_campaign($fresh_campaigns[$id]);
                    } else {
                        break; // Campaign deleted mid-run?
                    }
                    
                    // Add small delay to prevent API flooding/timeouts
                ...
                delete_transient($lock_key);
                 ...
                 // Campaign not due yet
                 // AIMatic_Logger::log("DEBUG: Campaign '{$campaign['name']}' not due. Next run in " . round($due_in/60) . " mins.", 'INFO');
            }
        }
    }
    ...
    private function run_campaign($campaign, $skip_images = false) {
        ...
        // CRITICAL FIX: Optimistic Scheduling
        // Update last_run immediately to prevent infinite loops if the run crashes or fails later.
        ...
        $instructions = $campaign['prompts'];
        if (!empty($keywords_str)) {
            $instructions .= "\n\nIMPORTANT: Optimize the article for the following SEO keywords: " . $keywords_str;
        }
        ...
        AIMatic_Logger::log("DEBUG: Article Generated (" . strlen($content) . " chars). Creating Post...", 'INFO');
        ...
        if ($media_success) {
             ...
             AIMatic_Logger::log("DEBUG: Media processing returned false/error. Publishing anyway as per user preference.", 'WARNING');
        }
